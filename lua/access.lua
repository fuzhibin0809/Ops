...
local token_secret   = ngx.var.ngo_token_secret or "1234567"
...
local function handle_token_uris(email, token, expires)
  if uri == "/_token.json" then
    ...
    ngx.say(json.encode({
      ...
      token   = token,
      expires = expires,
      refreshtoken = ngx.unescape_uri(ngx.var.cookie_OauthRefreshToken or ""),
    }))
    ngx.exit(ngx.OK)
  end
...
local function refreshtoken(token)
    local request = http.new()
    request:set_timeout(7000)
...
local function is_authorized()
  ...
  local Refreshtoken = ngx.unescape_uri(ngx.var.cookie_OauthRefreshToken or "")
  ...
  local expected_token = cb_server_name .. email .. expires
  if token == expected_token and expires and expires - 10 > ngx.time() then
    on_auth(email, expected_token , expires)
    return true
  elseif token == expected_token and expires and expires - 10 <= ngx.time() then
    local refresh, refresh_err = refreshtoken(Refreshtoken)
    expires = ngx.time() + refresh["expires_in"]
    token =  cb_server_name .. email .. expires
    local cookie_tail  = ";version=1;path=/;Max-Age=" .. expires
    if secure_cookies then
        cookie_tail = cookie_tail .. ";secure"
     end
    ngx.header["Set-Cookie"] = {
    "OauthEmail="       .. email .. cookie_tail,
    "OauthAccessToken=" .. token .. cookie_tail,
    "OauthRefreshToken=".. Refreshtoken .. cookie_tail,
...
local function authorize()
  ...
  local user_token = cb_server_name .. email .. expires
  ...
  ngx.header["Set-Cookie"] = {
    ...
    "OauthRefreshToken=".. ngx.escape_uri(token["refresh_token"]) .. cookie_tail,
    "OauthExpires="     .. expires .. cookie_tail,
  }
